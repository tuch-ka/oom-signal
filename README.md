...
```
oom-signal [--threshold=0.8] [--pid=1] [--signal=SIGUSR1] [--poll-interval=100ms] [--cooldown=5s]
```
...
2. Периодически читает `usage` / `limit` из cgroup. Интервал опроса адаптируется динамически: при росте потребления памяти интервал уменьшается (до 1ms), при стабилизации — мгновенно возвращается к базовому (`--poll-interval`).
...
- Скорость роста ≤ 0.5% limit/сек → интервал = `--poll-interval`
- Скорость роста ≥ 5% limit/сек → интервал = минимум (1ms)
- Между порогами → линейная интерполяция

Рост считается **в секунду**, а не за тик — это исключает осцилляцию при ускорении опроса. При снижении usage или отсутствии роста интервал мгновенно возвращается к `--poll-interval`.
...
```
[oom-signal] starting: threshold=0.80 pid=1 signal=SIGUSR1 poll-interval=100ms cooldown=5s
